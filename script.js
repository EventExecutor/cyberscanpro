class VirusTotalScanner {
    ...
    async pollForResults(analysisId, originalResource, type, maxAttempts = 20) {
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            try {
                ...
                console.log('Risposta polling:', data);

                if (data.data) {
                    const attributes = data.data.attributes;
                    if (attributes.status === 'completed') {
                        return this.formatScanResult(data.data, originalResource, type);
                    } else if (attributes.status === 'queued' || attributes.status === 'running') {
                        this.loadingText.textContent = `Scansione in corso... (${attributes.status}) (tentativo ${attempt + 1}/${maxAttempts})`;
                    } else {
                        throw new Error(`Stato analisi non riconosciuto: ${attributes.status}`);
                    }
                } else {
                    throw new Error(data.error?.message || 'Errore nel recupero risultati');
                }
            } catch (error) {
    ...
    async submitFileScan(file) {
        this.showToast('La scansione di file non è ancora supportata in questa modalità.', 'warning');
        throw new Error('Funzionalità di scansione file non implementata con il proxy.');
    }
    ...
    attachEventListeners() {
        ...
        this.apiModal.addEventListener('click', (e) => {
            if (e.target === this.apiModal) this.hideApiModal();
        });
    ...
    togglePasswordVisibility() {
        ...
        this.toggleApiKey.innerHTML = isPassword ? 
    ...
    async testApiConnection() {
        ...
        try {
            const response = await fetch(`${this.baseUrl}/users/${apiKey}`, {
                headers: {
                    'x-apikey': apiKey
                }
            });
            ...
            if (response.ok) {
                ...
                this.showToast('API key non valida o scaduta', 'error');
            }
        } catch (error) {
            this.showToast('Errore nella connessione: ' + error.message, 'error');
        } finally {
            this.testApiBtn.disabled = false;
            this.testApiBtn.innerHTML = '<i class="fas fa-vial"></i> Testa Connessione';
        }
    ...
    async handleFileSelect(file) {
        ...
        if (file.size > 650 * 1024 * 1024) {
            this.showToast('File troppo grande (max 650MB)', 'error');
            return;
        }
    ...
    async submitUrlScan(url) {
        console.log('Invio URL per scansione:', url);
        ...
        const data = await response.json();
        console.log('Risposta sottomissione URL:', data);
    ...
    async submitFileScan(file) {
        console.log('Invio file per scansione:', file.name);
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            try {
                ...
                if (response.ok && data.data) {
                    ...
                    if (attributes.status === 'completed') {
                        return await this.getDetailedResults(analysisId, originalResource, type);
                    } else if (attributes.status === 'queued' || attributes.status === 'running') {
                        this.loadingText.textContent = `Scansione in corso... (${attributes.status}) (tentativo ${attempt + 1}/${maxAttempts})`;
                        continue;
                    } else {
    ...
    async getDetailedResults(analysisId, originalResource, type) {
        const response = await fetch(`${this.baseUrl}/analyses/${analysisId}`, {
            headers: {
                'x-apikey': this.apiKey
            }
        ...
        if (response.ok && data.data) {
            ...
            throw new Error(data.error?.message || 'Errore nel recupero risultati dettagliati');
        }
    }
    ...
    formatScanResult(data, resource, type) {
        ...
        const engines = Object.entries(attributes.results || {}).map(([engine, result]) => ({
            engine,
        ...
        const total = Object.keys(attributes.results || {}).length;
        ...
        return {
            ...
            positives,
            total,
            permalink: `https://www.virustotal.com/gui/${type === 'url' ? 'url' : 'file'}/${data.id}`,
            status,
            engines,
            stats
        };
            ...
            <div class="result-card">
                <div class="result-header">
                    <div class="result-info">
                        <h3>${this.escapeHtml(result.resource)}</h3>
                        <p>Scansionato il ${new Date(result.scanDate).toLocaleString('it-IT')}</p>
                    </div>
                ...
                <div class="result-stats">
                    <div class="stat-item">
                        <div class="stat-value ${result.positives > 0 ? 'positive' : 'negative'}">
                            ${result.positives}
                        </div>
                    ...
                    <div class="stat-item">
                        ...
                        <div class="stat-label">Motori totali</div>
                    </div>
                    <div class="stat-item">
                        <div class="stat-value ${result.positives > 0 ? 'positive' : 'negative'}">
                            ${detectionRate}%
                        </div>
                        <div class="stat-label">Tasso rilevamento</div>
                    </div>
                </div>

                ${result.stats ? `
                    <div class="detailed-stats">
                        <h4>Dettagli Scansione:</h4>
                        <div class="stats-grid">
                            <div class="stat-detail malicious">
                                <span class="stat-number">${result.stats.malicious || 0}</span>
                                <span class="stat-name">Malevoli</span>
                            </div>
                            <div class="stat-detail suspicious">
                                <span class="stat-number">${result.stats.suspicious || 0}</span>
                                <span class="stat-name">Sospetti</span>
                            </div>
                            <div class="stat-detail clean">
                                <span class="stat-number">${result.stats.harmless || 0}</span>
                                <span class="stat-name">Puliti</span>
                            </div>
                            <div class="stat-detail undetected">
                                <span class="stat-number">${result.stats.undetected || 0}</span>
                                <span class="stat-name">Non rilevati</span>
                            </div>
                        </div>
                    </div>
                ` : ''}
                ...
                ${result.permalink ? `
                    <div style="margin-top: 20px;">
                        <a href="${result.permalink}" target="_blank" class="secondary-btn">
                            <i class="fas fa-external-link-alt"></i>
                            Vedi report completo su VirusTotal
                        </a>
                    </div>
                ` : ''}
            </div>
    ...
    renderHistory() {
        ...
        this.historyList.innerHTML = filteredHistory.map(item => `
            <div class="history-item" onclick="scanner.showHistoryDetails('${item.id}')">
                <div class="history-info">
                    <h4>${this.escapeHtml(item.resource)}</h4>
                    <p>${new Date(item.scanDate).toLocaleString('it-IT')} • ${item.positives}/${item.total} rilevamenti</p>
                </div>
                <div class="status-badge ${item.status}">
                    ${{clean: 'Pulito', infected: 'Infetto', suspicious: 'Sospetto'}[item.status]}
                </div>
            </div>
        `).join('');
    }
